...
def main(display_type, randomize, rings, nebula, border, width, height, noise, white_star, yellow_star, blue_star):
    import cairo
    import PIL
    import argparse
    import math
    ...
    width = 3000
    height = 2000
    rendering = display_type
    # rendering = 'orbit'
    # print(type(display_type)
    white_star = int(white_star)
    blue_star = int(blue_star)
    yellow_star = int(yellow_star)
    border_size = int(border)
    rand = randomize
    noise = float(noise)
    sun_size = random.randint(100, 400)
    make_rings = bool(rings)
    if nebula:
        print(nebula)
